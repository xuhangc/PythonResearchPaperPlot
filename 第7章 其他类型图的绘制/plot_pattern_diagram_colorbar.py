...
def plot_pattern_diagram_colorbar(X,Y,Z,option):
    ...
    #hp = plt.scatter(X,Y, s=markerSize, c=Z, marker = '*',edgecolors="k",linewidths=1) # 修改形状和边框颜色
    hp = plt.scatter(X,Y, s=markerSize, c=Z, marker = option['markersymbol'],zorder=10) # 修改形状和边框颜色
    ...
    for i,xval in enumerate(X):
        if type(option['markerlabel']) is list:
                    ...
                    xtextpos = X[i]+X[i]/30
                    ytextpos = Y[i]+Y[i]/30
                    text = plt.text(xtextpos,ytextpos,option['markerlabel'][i], 
                            #设置文本颜色和点相同
                             #color = option['markerlabelcolor'],
                             color=hp.to_rgba(Z[i]),
                             fontweight="bold",
